...
export default function BoardModal({ openModal, setModal }: any) {
  const [open, setOpen] = useState(true);
  const [title, setTitle] = useState<string>();
  const [content, setContent] = useState<string>();
  const [name, setName] = useState("NAME_ERROR");
  const [image, setImage] = useState("NAME_ERROR");
  const [email, setEmail] = useState("NAME_ERROR");
  ...
  function writeDB() {
    firebase.writeBoardData(database, title, content, name, image, email);
    setModal(!openModal);
  }
